...
if cassie:
    ...
    for op in xf.GetOrderedXformOps():
        if "translate" in op.GetOpName():
            ...
            print("Position: Z=1.15")
            break

# SPRINGY shock absorber setup:
# - Deep target angle (where it settles)
# - Medium stiffness (spring force to return)
# - High damping (absorbs impact, prevents bouncing)

joint_config = {
    "hip_abduction": {"stiff": 5000, "damp": 1000, "target": 0},
    "hip_rotation": {"stiff": 5000, "damp": 1000, "target": 0},
    "hip_flexion": {"stiff": 8000, "damp": 1500, "target": 20},
    "knee_joint": {"stiff": 8000, "damp": 2000, "target": -50},       # Deeper bend
    "knee_to_shin": {"stiff": 8000, "damp": 2000, "target": -40},     # Deeper
    "ankle_joint": {"stiff": 10000, "damp": 3000, "target": 120},     # MUCH deeper - 120°
    "toe_joint": {"stiff": 8000, "damp": 1500, "target": -120},       # Compensate for flat
}

print("\\nSHOCK ABSORBER setup:")
print("  Lower stiffness = more flex on impact")
print("  Higher damping = absorbs energy smoothly")
print("")
...
for prim in stage.Traverse():
    ...
    if prim.HasAPI(UsdPhysics.DriveAPI):
        ...
        if drive:
            config = {"stiff": 5000, "damp": 1000, "target": 0}
            ...
            if abs(config["target"]) > 0:
                print(f"  {name}: target={config['target']}°, stiff={config['stiff']}, damp={config['damp']}")

print("")
print("Ankle: 120° deep bend")
print("Damping 2000-3000 for smooth absorption")
...
print("Setting up springy shock-absorber knees...")
