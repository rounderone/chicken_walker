# Cassie with nearly straight legs to reach ground
import socket
import json
import time
...
# Position - Cassie with straight legs is about 1m tall
# Put pelvis at 0.95m so feet are just above ground
cassie_prim = stage.GetPrimAtPath("/World/Cassie")
if cassie_prim:
    xf = UsdGeom.Xformable(cassie_prim)
    for op in xf.GetOrderedXformOps():
        if "translate" in op.GetOpName():
            op.Set(Gf.Vec3d(0, 0, 0.95))
            print("Cassie pelvis at Z=0.95")
            break

# Make pelvis KINEMATIC for testing
...
# Set joint targets - values in RADIANS (not degrees!)
# Small angles to keep legs nearly straight
import math

print("\\nSetting joint targets (in radians)...")

joint_targets = {
    "hip_flexion_left": math.radians(10),      # ~0.17 rad - slight forward
    "hip_flexion_right": math.radians(10),
    "knee_joint_left": math.radians(-15),      # ~-0.26 rad - slight bend
    "knee_joint_right": math.radians(-15),
    "ankle_joint_left": math.radians(10),      # Compensate
    "ankle_joint_right": math.radians(10),
    "toe_joint_left": math.radians(-5),        # Keep flat
    "toe_joint_right": math.radians(-5),
}

# Higher stiffness to hold position against gravity
STIFFNESS = 1000
DAMPING = 100
...
for prim in stage.Traverse():
    ...
    if prim.HasAPI(UsdPhysics.DriveAPI):
        ...
        if drive:
            ...
            if target != 0:
                print(f"  {name}: {math.degrees(target):.1f} deg ({target:.3f} rad)")
...
print("\\n=== Press PLAY ===")
print("Legs should extend down to ground")
'''

print("Setting up Cassie with straight legs...")
